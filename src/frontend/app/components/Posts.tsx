...
const Post = ({post}: {post: AllPostsQueryResult[number]}) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mt-6 pt-4 border-t border-brown/10">
        {author && author.firstName && author.lastName && (
          <div className="flex items-center">
            <Avatar person={author} small={true} />
          </div>
        )}
        <time className="text-brown/60 text-xs font-mono" dateTime={date}>
...
const Posts = ({
  ...
  <div>
    {heading && (
      <h2 className="font-perandory text-3xl font-normal tracking-tight text-brown sm:text-4xl lg:text-5xl">
        {heading}
      </h2>
    )}
    {subHeading && <p className="mt-2 text-lg leading-8 text-brown/70 font-rokkitt">{subHeading}</p>}
